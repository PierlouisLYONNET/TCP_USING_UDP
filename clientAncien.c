...
int main()
{
    ...
    printf("\n\n\n---------- Send : ----------\n%s\n", buffer_data);
    sendto(server_socket, (char *)buffer_data, strlen(buffer_data), MSG_CONFIRM, (const struct sockaddr *)&servaddr, sizeof(servaddr));

    char buffer_sequence[(SIZE_HEADER + 1)]; // LE +1 c good
    ...
    while (end_transmission)
    {
        ...
        memset(buffer_data, 0, sizeof(buffer_data)); // TODO OPTIMIZE
        memset(buffer_sequence, 0, sizeof(buffer_sequence));
        memset(temp_buffer, 0, sizeof(temp_buffer));

        n = recvfrom(server_socket, (char *)buffer_data, MAXLINE, MSG_WAITALL, (struct sockaddr *)&addr, &len);
        buffer_data[n] = '\0';
        ...
        sendto(server_socket, (char *)buffer_ack, strlen(buffer_ack), MSG_CONFIRM, (const struct sockaddr *)&servaddr, sizeof(servaddr));
        ...
        if(sequence_number_current != 0) {
            ...
            printf("J'écris\n\n");
            fwrite(buffer_data, 1, (n - SIZE_HEADER), file_ptr);
        } else {
            printf("STOP\n\n");
            end_transmission = 0;
        }
